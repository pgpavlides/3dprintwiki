...
function PrintCostCalculator() {
  // Create a ref for the print weight input
  const printWeightInputRef = useRef<HTMLInputElement>(null);
  
  // Basic inputs
  const [weight, setWeight] = useState<number | "">("") // Empty string to show blank field
  ...
  // Use effect to focus the print weight input when component mounts
  useEffect(() => {
    // Add event listener for 'S' key to focus print weight field
    const handleKeyDown = (e: KeyboardEvent) => {
      // Check if 'S' key was pressed (and not in an input or textarea)
      ...
      if (e.key === 's' && !isInputField) {
        e.preventDefault();
        if (printWeightInputRef.current) {
          printWeightInputRef.current.focus();
          printWeightInputRef.current.select();
        }
      }
    };
  ...
  const materialCost = (weight === "" ? 0 : Number(weight) * filamentCost) / 1000
